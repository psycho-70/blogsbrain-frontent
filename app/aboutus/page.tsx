        ...
        <div className={`min-h-screen ${isDark ? 'bg-transparent' : 'bg-gradient-to-br from-slate-50 via-purple-50/30 to-blue-50/40'}`}>
            ...
            <section className="relative py-32 overflow-hidden">
                ...
                <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 z-10">
                    ...
                    >
                        ...
                        >
                            Founded in 2015, we've grown from a small blog to one of the most trusted <span className={`font-semibold underline decoration-purple-500/50 ${isDark ? 'text-white' : 'text-purple-700'}`}>content platforms</span>, reaching millions worldwide.
                        </motion.p>
                        <motion.div variants={itemVariants} className="flex justify-center space-x-6">
                            <Link
                                href="/contactus"
                                className={`relative group overflow-hidden px-10 py-4 rounded-xl font-bold transition-all shadow-2xl ${
                                    isDark
                                        ? 'bg-gradient-to-r from-blue-600 to-purple-600 text-white hover:shadow-purple-500/40'
                                        : 'bg-gradient-to-r from-blue-500 to-purple-500 text-white hover:shadow-purple-400/50'
                                }`}
                            >
                            ...
                            <Link
                                href="#team"
                                className={`border-2 px-10 py-4 rounded-xl font-bold transition-all backdrop-blur-sm ${
                                    isDark
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    ...
                    >
                        {stats.map((stat, index) => (
                            <div key={index} className={`text-center p-6 rounded-2xl border transition-colors ${
                                isDark
                                    ? 'bg-gray-900/50 backdrop-blur-sm border-gray-800 hover:border-purple-500/30'
                                    : 'bg-white/70 backdrop-blur-sm border-gray-200 hover:border-purple-400/50 shadow-sm'
                            }`}>
                                <div className={`inline-flex items-center justify-center w-16 h-16 rounded-full mb-4 ${
                                    ...
                                    <stat.icon className={`w-8 h-8 ${isDark ? 'text-blue-400' : 'text-blue-600'}`} />
                                </div>
                                <div className={`text-4xl font-bold mb-2 ${isDark ? 'text-white' : 'text-slate-900'}`}>
                                    {stat.number}
                                </div>
                                <div className={isDark ? 'text-gray-400' : 'text-slate-500'}>
                                    {stat.label}
                                </div>
                            </div>
                        ))}
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="grid md:grid-cols-2 gap-12 items-center">
                        ...
                        >
                            ...
                            <p className={`mb-6 leading-relaxed ${isDark ? 'text-gray-300' : 'text-slate-600'}`}>
                        ...
                        >
                            ...
                            <div className={`absolute inset-0 bg-gradient-to-t ${
                                isDark ? 'from-gray-900 via-transparent to-transparent' : 'from-white/50 via-transparent to-transparent'
                            }`}></div>
                        </motion.div>
                    </div>
                </div>
                    ...
                    <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-8">
                        {teamMembers.map((member, index) => (
                            <motion.div
                                ...
                                viewport={{ once: true }}
                                transition={{ delay: index * 0.1 }}
                                className={`group text-center p-6 rounded-2xl border transition-all hover:-translate-y-2 ${
                                    isDark
                                        ? 'bg-gray-900/30 border-gray-800 hover:border-blue-500/50'
                                        : 'bg-white/60 border-gray-200 hover:border-blue-400/50 shadow-sm'
                                }`}
                            >
                                <div className={`relative w-40 h-40 mx-auto mb-6 rounded-full overflow-hidden ring-4 transition-colors shadow-lg ${
                                    isDark 
                                        ? 'ring-gray-800 group-hover:ring-blue-500' 
                                        : 'ring-gray-200 group-hover:ring-blue-400'
                                }`}>
                                    <Image
                                        src={member.image}
                                        alt={member.name}
                                ...
                                <p className={`mb-4 text-sm ${isDark ? 'text-gray-400' : 'text-slate-500'}`}>
                                    {member.bio}
                                </p>
                                <div className="flex justify-center space-x-4">
                                    <a href={member.social.twitter} className={`transition-colors ${
                                    ...
                                    <a href={member.social.linkedin} className={`transition-colors ${
                                        isDark ? 'text-gray-500 hover:text-blue-600' : 'text-gray-400 hover:text-blue-700'
                                    }`}>
            ...
            <section className="py-20 relative overflow-hidden">
                <div className={`absolute inset-0 ${
                    isDark 
                        ? 'bg-gradient-to-r from-blue-900/50 to-purple-900/50 backdrop-blur-md' 
                        : 'bg-gradient-to-r from-blue-100/80 to-purple-100/80 backdrop-blur-sm'
                }`}></div>
                <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center relative z-10">
                    <h2 className={`text-3xl font-bold mb-6 ${isDark ? 'text-white' : 'text-slate-900'}`}>
                        Ready to Join Our Journey?
                    </h2>
                    <p className={`mb-8 text-lg ${isDark ? 'text-gray-300' : 'text-slate-600'}`}>
                        Whether you want to contribute, collaborate, or just learn more about what we do,
                        we'd love to hear from you.
                    </p>
                    <Link
                        href="/contactus"
                        className={`inline-flex items-center px-8 py-4 rounded-lg font-bold transition-colors text-lg ${
                            isDark 
